...
const knownChatNames = ['sampleChat'];
//const chatName = process.argv[2];
const chatName = 'sampleChat'
...
client
  ...
  onCreateAssistantResponse${capitalizedChatName}(conversationId: "${conversationId}") {
    content {
      text
    }
  }
...
for await (const line of reader) {
  ...
  await client.mutate({
    mutation: gql(
      `
    mutation MyMutation {
  ${chatName}(content: {text: "${line}"}, conversationId: "${conversationId}") {
    id
  }
}
    `
