  ...
  implements LambdaEventSource
{
  ...
  constructor(
    ...
    private readonly cdk: AmplifyCdkType
  ) {
    super(scope, name);
  }
  ...
  finalize(): void {
    ...
    const ddbManagerPolicy = new this.cdk.aws_iam.Policy(
      this,
      "createUpdateDeleteTablesPolicy"
    );
    ...
    const lambdaCode = this.cdk.aws_lambda.Code.fromAsset(
      "./lib/ddb-custom-handler"
    );

    // lambda that will handle DDB CFN events
    const gsiOnEventHandler = new this.cdk.aws_lambda.Function(
      this,
      "tableOnEventHandler",
      {
    ...
    const gsiIsCompleteHandler = new this.cdk.aws_lambda.Function(
      this,
      "tableIsCompleteHandler",
      {
        runtime: this.cdk.aws_lambda.Runtime.NODEJS_16_X,
        code: lambdaCode,
    ...
    const gsiCustomProvider = new this.cdk.custom_resources.Provider(
      this,
      "tableCustomProvider",
      {
