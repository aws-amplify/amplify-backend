...
/**
 * If true, or if a settings object is provided, the MFA token is sent to the user via EMAIL to their verified email address.
 * @see - https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-advanced-security-email-mfa.html
 */
export type MFAEmailSettings =
  | boolean
  | {
      /**
       * The Email message template sent during MFA verification.
       * Use the code parameter in the template where Cognito should insert the verification code.
       * @default
       * emailMessage: (createCode: string) => `Your authentication code is ${createCode()}.`
       */
      emailMessage: (createCode: () => string) => string;
      emailSubject: string;
    };
/**
...
/**
 * Configure the MFA types that users can use. At least one MFA setting is required.
 */
export type MFASettings = RequireAtLeastOne<{
  totp: MFATotpSettings;
  sms: MFASmsSettings;
  email: MFAEmailSettings;
}>;
export type MFAType = MfaSecondFactor & {
  /**
   * The MFA token is sent to the user via email to their verified email address
   * @see https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-advanced-security-email-mfa.html
   * @default false
   */
  email: boolean;
