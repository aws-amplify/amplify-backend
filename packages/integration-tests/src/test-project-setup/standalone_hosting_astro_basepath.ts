...
/**
 * Creates a real Astro 5 server-output hosting project configured with
 * `base: '/app'` and `trailingSlash: 'always'` so that deployment tests
 * exercise the manifest.basePath end-to-end + trailing-slash canonical
 * redirect paths through CloudFront.
 *
 * Mirrors StandaloneHostingAstroTestProjectCreator: copies real Astro source
 * code into a fresh tmpdir, then runs `npm install` so `astro build` can
 * execute during deployment.
 */
export class StandaloneHostingAstroBasepathTestProjectCreator implements TestProjectCreator {
  readonly name = 'standalone-hosting-astro-basepath';

  /**
   * Creates a new StandaloneHostingAstroBasepathTestProjectCreator instance.
   */
  constructor(
  ...
  createProject = async (e2eProjectDir: string): Promise<TestProjectBase> => {
    ...
    const project = new StandaloneHostingAstroBasepathTestProject(
      projectName,
      projectRoot,
      projectAmplifyDir,
      this.cfnClient,
...
class StandaloneHostingAstroBasepathTestProject extends TestProjectBase {
  readonly sourceProjectDirPath =
    '../../src/test-projects/standalone-hosting-astro-basepath';
