 ...
 * The line is something like "at SymbolName (/some/path/to/project/root/backend/resource.ts:3:21)"
 * or on Windows "at SymbolName (C:\some\path\to\project\root\backend\resource.ts:3:21)"
 *
 * This regex pulls out the file path into the "filepath" named matching group
 * In the above examples that would be "/some/path/to/project/root/backend/resource.ts" or "C:\some\path\to\project\root\backend\resource.ts", respectively
 *
 * Note: this regex is duplicated in packages/backend/src/engine/extract_file_path_from_stack_trace_line.ts
