...
void describe('FunctionModelIntrospectionSchemaGenerator', () => {
  void it('generates a client configuration file', () => {
    ...
    const functionEnvironmentTypeGenerator =
      new FunctionModelIntrospectionSchemaGenerator('testFunction');
    const sampleStaticEnv = '_HANDLER: string;';

    functionEnvironmentTypeGenerator.generateModelIntrospectionSchema('{}');
  ...
  void it('generated client configuration file has valid syntax', async () => {
    ...
    JSON.parse(pathToFileURL(filePath).toString());
