...
export class FunctionEnvironmentTypeGenerator {
  ...
  private readonly envPreAssignment = 'export const env: any = process.env';
  ...
  generateProcessEnvShim = () => {
    ...
    if (fs.existsSync(this.typeDefFilePath)) {
      return;
    }

    const content = `${this.header}${EOL}${this.envPreAssignment};`;
    this.writeShimFile(content);
  };
