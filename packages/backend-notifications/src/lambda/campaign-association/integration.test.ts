...
void describe('resolveCampaignsServiceLinkedRoleArn', () => {
  void it('selects the connect-campaigns SLR by RoleName regardless of list position (not first, not last)', async () => {
    // The real SLR sits in the MIDDLE, flanked by decoys. This distinguishes
    // RoleName matching from BOTH take-first (index 0) and take-last
    // (roles[length - 1]) selection — only a name match returns the right ARN.
    const rec = recorder(() => ({
      Roles: [
        {
          RoleName: 'DecoyFirstRole',
          Path: '/aws-service-role/connect-campaigns.amazonaws.com/',
          Arn: 'arn:aws:iam::x:role/aws-service-role/connect-campaigns.amazonaws.com/DecoyFirst',
        },
        {
        ...
        {
          RoleName: 'DecoyLastRole',
          Path: '/aws-service-role/connect-campaigns.amazonaws.com/',
          Arn: 'arn:aws:iam::x:role/aws-service-role/connect-campaigns.amazonaws.com/DecoyLast',
        },
      ],
    }));
  ...
  void it('falls back to the last role when none match the expected RoleName (degrades safely)', async () => {
    const fallbackArn =
