...
export const compileProject = (projectDirectory: string) => {
  ...
  if (!ts.sys || typeof ts.readConfigFile !== 'function') {
    throw new AmplifyUserError('UnsupportedTypeScriptVersionError', {
      message:
        "The installed 'typescript' version does not expose the JavaScript Compiler API required to type-check your backend.",
      resolution:
