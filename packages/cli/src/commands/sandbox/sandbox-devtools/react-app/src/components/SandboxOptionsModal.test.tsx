...
describe('SandboxOptionsModal Component', () => {
  ...
  it('disables "once" checkbox when streamFunctionLogs is checked', async () => {
    ...
    const streamLogsCheckbox = Array.from(
      container.querySelectorAll('input[type="checkbox"]'),
    ).find((input) =>
      input
        .closest('label')
        ?.textContent?.includes('Stream function execution logs'),
    );

    const onceCheckbox = Array.from(
      container.querySelectorAll('input[type="checkbox"]'),
    ).find((input) =>
      input
        .closest('label')
        ?.textContent?.includes('Execute a single deployment'),
    );
