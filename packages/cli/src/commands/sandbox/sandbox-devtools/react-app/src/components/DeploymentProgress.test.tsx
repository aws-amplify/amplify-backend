  ...
  const mockService = {
    ...
    onSavedCloudFormationEvents: vi.fn((handler) => {
      ...
      return { unsubscribe: vi.fn(() => {
        ...
        if (index !== -1) subscribers.savedCloudFormationEvents.splice(index, 1);
      }) };
    }),
    ...
    onCloudFormationEventsError: vi.fn((handler) => {
      ...
      return { unsubscribe: vi.fn(() => {
        ...
        if (index !== -1) subscribers.cloudFormationEventsError.splice(index, 1);
      }) };
    }),
    ...
    emitEvent: (eventName: string, data: any) => {
      if (eventName === 'cloudFormationEvents') {
        ...
        subscribers.savedCloudFormationEvents.forEach(handler => handler(data));
      } else if (eventName === 'cloudFormationEventsError') {
        subscribers.cloudFormationEventsError.forEach(handler => handler(data));
...
describe('DeploymentProgress Component', () => {
  ...
  it('renders with correct header', () => {
    render(
      <DeploymentProgress
        deploymentClientService={mockDeploymentService as unknown as DeploymentClientService}
        visible={true}
        status="running"
      />
    );
  ...
  it('requests CloudFormation events on mount', () => {
    ...
    // Verify API calls
    expect(mockDeploymentService.getSavedCloudFormationEvents).toHaveBeenCalled();
  ...
  it('sets up polling when deployment is in progress', () => {
    ...
    // Initial call
    expect(mockDeploymentService.getCloudFormationEvents).toHaveBeenCalledTimes(1);
  ...
  it('displays deployment events when received', () => {
    ...
    act(() => {
      mockDeploymentService.emitEvent('savedCloudFormationEvents', sampleEvents);
    });
  ...
  it('expands section automatically when deployment starts', () => {
    ...
    expect(screen.getByText('Waiting for deployment events...')).toBeInTheDocument();
  });
