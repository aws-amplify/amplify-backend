...
vi.mock('./contexts/socket_client_context', async () => {
  ...
  return {
    ...
    SocketClientProvider: ({ children }: { children: React.ReactNode }) => <>{children}</>,
...
vi.mock('@cloudscape-design/components', () => ({
  AppLayout: ({ content }: { content: React.ReactNode }) => <div data-testid="app-layout">{content}</div>,
  ...
  Alert: ({ 
    ...
    <div data-testid={`alert-${type}`} role="alert">
      ...
      {dismissible && (
        <button data-testid="alert-dismiss" aria-label="Dismiss" onClick={onDismiss}>
          Dismiss
        </button>
      )}
    </div>
...
vi.mock('./components/Header', () => ({
  default: vi.fn((props: {
    ...
    <div data-testid="header-component">
      ...
      <button onClick={props.onStartSandbox} disabled={props.isStartingLoading}>
        Start Sandbox
      </button>
      <button onClick={props.onStopSandbox} disabled={props.isStoppingLoading}>
        Stop Sandbox
      </button>
      <button onClick={props.onDeleteSandbox} disabled={props.isDeletingLoading}>
        Delete Sandbox
      </button>
...
vi.mock('./components/DeploymentProgress', () => ({
  default: vi.fn(({ 
    ...
    <div data-testid="deployment-progress" style={{ display: visible ? 'block' : 'none' }}>
...
vi.mock('./components/SandboxOptionsModal', () => ({
  default: vi.fn(({ 
    ...
    <div data-testid="sandbox-options-modal" style={{ display: visible ? 'block' : 'none' }}>
      <button 
      ...
      <button 
        data-testid="sandbox-options-cancel"
        onClick={onDismiss}
      >
        Cancel
      </button>
...
vi.mock('./components/LogSettingsModal', () => ({
  default: vi.fn(({ 
    ...
    <div data-testid="log-settings-modal" style={{ display: visible ? 'block' : 'none' }}>
      ...
      <button 
        data-testid="log-settings-cancel"
        onClick={onDismiss}
      >
        Cancel
      </button>
      <button 
        data-testid="log-settings-clear"
        onClick={onClear}
      >
        Clear Logs
      </button>
...
describe('App Component', () => {
  ...
  const createMockSandboxService = () => {
    ...
    return {
      ...
      emitLog: (logData: any) => {
        ...
        const logWithId = {
          ...logData,
          id: logData.id || `log-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
        };
      ...
      emitReconnectAttempt: (attemptNumber: number) => {
        subscribers.reconnectAttempt.forEach((handler) => handler(attemptNumber));
      },
  ...
  it('updates state on sandbox status change', async () => {
    ...
    expect(within(headerElement).getByText(/Status: running/i)).toBeInTheDocument();
    
    // Check localStorage was updated
    expect(window.localStorage.setItem).toHaveBeenCalledWith('sandboxIdentifier', 'sandbox-123');
    ...
    expect(window.localStorage.removeItem).toHaveBeenCalledWith('sandboxIdentifier');
  });
  ...
  it('handles tab switching', async () => {
    ...
    expect(screen.getByTestId('tab-resources')).toHaveAttribute('aria-selected', 'false');
    expect(screen.getByTestId('console-viewer')).toBeInTheDocument();
    ...
    expect(screen.getByTestId('tab-logs')).toHaveAttribute('aria-selected', 'false');
    expect(screen.getByTestId('tab-resources')).toHaveAttribute('aria-selected', 'true');
  ...
  it('handles connection errors', async () => {
    ...
    expect(screen.getByText(/DevTools process was interrupted/i)).toBeInTheDocument();
  ...
  it('handles deployment completion events', async () => {
    ...
    expect(screen.getByText(/Deployment completed successfully/i)).toBeInTheDocument();
  });
  ...
  it('handles modal dismissal', async () => {
    ...
    // Check modal is closed
    expect(screen.getByTestId('sandbox-options-modal')).toHaveStyle({ display: 'none' });
