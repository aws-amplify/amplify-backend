...
describe('Resource Management and Error Recovery Flow', () => {
  ...
  it('tests complete user journey with cross-component interaction and error recovery', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/DevTools connected to Amplify Sandbox/i)).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      ...
      expect(within(headerElement).getByText(/No Sandbox/i)).toBeInTheDocument();
    });
    ...
    const startSandboxButton = within(modalElement).getByRole('button', { name: /Start/i });
    ...
    await waitFor(() => {
      expect(screen.getByText(/Waiting for deployment events/i)).toBeInTheDocument();
    });
    ...
    const mockResources = [
      {
        ...
        consoleUrl: 'https://console.aws.amazon.com/lambda/home?region=us-east-1#/functions/lambda-1',
      ...
      },
      {
        ...
        consoleUrl: 'https://console.aws.amazon.com/apigateway/home?region=us-east-1',
        logGroupName: null
      }
    ];
    ...
    devToolsServer.emitLog(LogLevel.ERROR, 'Error executing function: Memory limit exceeded');
    ...
    const deleteButton = screen.getByRole('button', { name: /Delete Sandbox/i });
    await user.click(deleteButton);
    
    // Verify delete confirmation modal appears
    await waitFor(() => {
      expect(screen.getByText(/Are you sure you want to delete the sandbox\?/i)).toBeInTheDocument();
    });
    
    // Confirm delete - use within to get the button inside the modal
    const modalDialog = screen.getByTestId('modal');
    const confirmButton = within(modalDialog).getByRole('button', { name: /^Delete$/i });
