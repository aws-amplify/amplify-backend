...
describe('Console Logs Integration', () => {
  ...
  it('handles sandbox state changes and logs them', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sandbox status changed to: running/i)).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sandbox status changed to: deploying/i)).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      const runningLogs = screen.getAllByText(/Sandbox status changed to: running/i);
  ...
  it('tests log filtering by level', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText(/Warning log message/i)).not.toBeInTheDocument();
      expect(screen.queryByText(/Success log message/i)).not.toBeInTheDocument();
    });
  });
  ...
  it('tests sandbox lifecycle operations', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sandbox stopped successfully/i)).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sandbox deleted successfully/i)).toBeInTheDocument();
    }, { timeout: 2000 });
  });
});
