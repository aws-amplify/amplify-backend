...
describe('ResourceConsole Component', () => {
  ...
  it('renders resources when sandbox is running', () => {
    const { getByText, getAllByText } = render(
      <ResourceConsole sandboxStatus="running" />,
    );
  ...
  it('shows deploying state when sandbox is deploying', () => {
    ...
    expect(
      getByText(/The sandbox is currently being deployed/),
    ).toBeInTheDocument();
  });

  it('shows nonexistent state when sandbox does not exist', () => {
    const { getByText } = render(
      <ResourceConsole sandboxStatus="nonexistent" />,
    );
  ...
  it('refreshes resources when refresh button is clicked', async () => {
    ...
    vi.mocked(useResourceManager).mockReturnValue(
      createBaseMock({
  ...
  it('opens edit modal and completes the editing workflow', async () => {
    ...
    const {
      ...
      getAllByText,
      getByDisplayValue,
    } = render(<ResourceConsole sandboxStatus="running" />);
    ...
    expect(mockUpdateFriendlyName).toHaveBeenCalledWith(
      'lambda1',
      'My Custom Function',
    );
  });
