...
describe('ResourceLogPanel Component', () => {
  ...
  const createMockLoggingService = () => {
    ...
    return mockService as unknown as LoggingClientService & {
      emitEvent: typeof mockService.emitEvent;
    };
  };

  // Setup default props for the component
  const createDefaultProps = (
    mockLoggingService: ReturnType<typeof createMockLoggingService>,
  ) => ({
  ...
  it('requests logs on mount', () => {
    ...
    expect(mockLoggingService.viewResourceLogs).toHaveBeenCalledWith(
      'lambda-123',
    );
  });
  ...
  it('shows AWS console link when URL is provided', () => {
    ...
    const consoleLink = container.querySelector(
      'a[href="https://console.aws.amazon.com/lambda"]',
    );
    ...
    act(() => {
      mockLoggingService.emitEvent('lambdaTestResult', {
        ...
        result: JSON.stringify({
          ...
          trace: [
            'at handler (/var/task/index.js:5:20)',
            'at Runtime.handleMessage',
          ],
        }),
      });
    });
  ...
  it('persists logs between component instances', () => {
    ...
    // First render
    const { unmount, container: firstContainer } = render(
      <ResourceLogPanel {...props} />,
    );
    ...
    act(() => {
      mockLoggingService.emitEvent('savedResourceLogs', {
        ...
        logs: [
          {
            timestamp: '2023-01-01T12:00:00Z',
            message: 'New log after recovery',
          },
        ],
      });
    });
    ...
    act(() => {
      mockLoggingService.emitEvent('savedResourceLogs', {
        ...
        logs: [
          {
            timestamp: '2023-01-01T12:00:00Z',
            message: 'Selectable log content',
          },
        ],
      });
    });
